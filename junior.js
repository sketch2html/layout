...
const model = require('./model/junior');
...
dataes.forEach(item => {
  let { data, classify, direction, row, col, area, directions } = item;
  x.push(model(data, row, col, direction, area, directions));
  y.push(classify);
});
...
let list = [];
dataes.forEach(item => {
  let { id, data, classify, direction, row, col, area, directions } = item;
  let param = model(data, row, col, direction, area, directions);
  let res = f([param], w, b);
